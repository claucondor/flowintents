...
/// @title FlowIntentsComposerV2
/// @notice Dual-chain intent submission + strategy execution for the FlowIntents solver protocol.
///         EVM users deposit funds here; Cadence ScheduledManager polls and picks them up.
///         Solvers execute yield strategies via COA-authorized batch calls.
...
contract FlowIntentsComposerV2 is Ownable, ReentrancyGuard {
    ...
    struct EVMIntentRequest {
        ...
        uint256 amount;
        uint256 targetAPY;      // basis points (500 = 5%)
        uint256 durationDays;
        uint8 principalSide;    // 0=EVM_YIELD, 1=CADENCE_YIELD
        uint256 submittedAt;
    ...
    modifier onlyCOA() {
        require(msg.sender == authorizedCOA, "FlowIntentsComposerV2: not COA");
        _;
    }
    ...
    function setAuthorizedCOA(address _coa) external onlyOwner {
        require(_coa != address(0), "FlowIntentsComposerV2: zero COA");
    ...
    // -------------------------------------------------------------------------
    // Intent Submission (EVM users)
    // -------------------------------------------------------------------------

    /// @notice Submit an intent from the EVM side. For native FLOW: send value with token=address(0).
    ...
    function submitIntent(
        ...
        require(targetAPY > 0, "FlowIntentsComposerV2: zero APY");
        require(durationDays > 0, "FlowIntentsComposerV2: zero duration");
        require(principalSide <= 1, "FlowIntentsComposerV2: invalid principalSide");

        uint256 depositAmount;

        if (token == address(0)) {
            // Native FLOW deposit
            require(msg.value > 0, "FlowIntentsComposerV2: no FLOW sent");
            depositAmount = msg.value;
        } else {
            // ERC-20 deposit
            require(amount > 0, "FlowIntentsComposerV2: zero amount");
            require(msg.value == 0, "FlowIntentsComposerV2: no ETH for ERC20");
    ...
    function markPickedUp(uint256 intentId) external onlyCOA {
        require(intentId > 0 && intentId < nextIntentId, "FlowIntentsComposerV2: invalid intentId");
        EVMIntentRequest storage req = _intentRequests[intentId];
        require(!req.pickedUp, "FlowIntentsComposerV2: already picked up");
        require(intentStatuses[intentId] == IntentStatus.PENDING, "FlowIntentsComposerV2: not pending");
    ...
    function executeStrategy(
        ...
        require(
            ...
            "FlowIntentsComposerV2: not ready for execution"
        );

        // Decode the strategy steps
        StrategyStep[] memory steps = abi.decode(encodedBatch, (StrategyStep[]));
        require(steps.length > 0, "FlowIntentsComposerV2: empty batch");
        ...
        for (uint256 i = 0; i < steps.length; ) {
            ...
            if (!ok) {
                ...
                revert("FlowIntentsComposerV2: strategy step failed");
            }
    ...
    /// @notice Mark an intent as completed. Called by COA after yield is returned.
    ...
    function markCompleted(uint256 intentId, uint256 finalBalance) external onlyCOA {
        ...
        require(
            intentStatuses[intentId] == IntentStatus.EXECUTING ||
            intentStatuses[intentId] == IntentStatus.PICKED_UP,
            "FlowIntentsComposerV2: not executing"
        );
    ...
    /// @notice Withdraw funds — user claims back if cancelled, or claims yield if completed.
    /// @param intentId The intent to withdraw from
    function withdraw(uint256 intentId) external nonReentrant {
        require(intentId > 0 && intentId < nextIntentId, "FlowIntentsComposerV2: invalid intentId");
        EVMIntentRequest storage req = _intentRequests[intentId];
        require(req.user == msg.sender, "FlowIntentsComposerV2: not intent owner");
        require(
            intentStatuses[intentId] == IntentStatus.COMPLETED ||
            intentStatuses[intentId] == IntentStatus.CANCELLED,
            "FlowIntentsComposerV2: not withdrawable"
        );

        uint256 amount = intentBalances[intentId];
        require(amount > 0, "FlowIntentsComposerV2: nothing to withdraw");
        ...
        if (req.token == address(0)) {
            ...
            require(sent, "FlowIntentsComposerV2: FLOW transfer failed");
        } else {
            // ERC-20
            IERC20(req.token).safeTransfer(msg.sender, amount);
        }
    ...
    function quoteLZBridge(
        ...
        return ILayerZeroEndpoint(LAYERZERO_ENDPOINT).quote(
            dstEid,
            message,
            options,
