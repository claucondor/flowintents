...
access(all) contract ScheduledManagerV0_3 {
    ...
    access(all) resource Admin {
        access(all) fun setRebalanceThreshold(threshold: UFix64) {
            ...
            ScheduledManagerV0_3.rebalanceThreshold = threshold
        }

        access(all) fun setEVMContract(name: String, config: EVMConfig) {
            ScheduledManagerV0_3.evmConfig[name] = config
        ...
        access(all) fun getEVMConfig(name: String): EVMConfig? {
            return ScheduledManagerV0_3.evmConfig[name]
        }
        ...
        access(all) fun checkFallbackExecution(
            ...
            let bidSelectedIDs = IntentMarketplaceV0_3.getBidSelectedIntents()
            let currentBlock = getCurrentBlock().height

            let marketplace = getAccount(ScheduledManagerV0_3.account.address)
        ...
        /// Poll EVM for pending intents and create them in IntentMarketplaceV0_3.
        ...
        access(all) fun pollEVMIntents(coaRef: auth(EVM.Call) &EVM.CadenceOwnedAccount) {
            let config = ScheduledManagerV0_3.evmConfig["composerV2_getPendingIntents"]
                ?? panic("EVMConfig not set for composerV2_getPendingIntents")

            let composerAddr = ScheduledManagerV0_3.parseEVMAddress(config.address)
            ...
            let idsOffset = ScheduledManagerV0_3.decodeUInt256(data: data, offset: 0)
            if Int(idsOffset) + 32 > data.length {
                emit EVMPollCompleted(intentsPolled: 0, intentsCreated: 0, timestamp: getCurrentBlock().timestamp)
                return
            }

            let count = ScheduledManagerV0_3.decodeUInt256(data: data, offset: Int(idsOffset))
            if count == 0 {
                emit EVMPollCompleted(intentsPolled: 0, intentsCreated: 0, timestamp: getCurrentBlock().timestamp)
                return
            }
            ...
            while i < count {
                ...
                let evmIntentId = ScheduledManagerV0_3.decodeUInt256(data: data, offset: idOffset)
                ...
                let cadenceIntentId = marketplace.createEVMIntent(
                    ownerAddress: ScheduledManagerV0_3.account.address,
                    vault: <- emptyVault,
                ...
                let markConfig = ScheduledManagerV0_3.evmConfig["composerV2_markPickedUp"]
    ...
    access(all) fun checkActiveIntents(intentIDs: [UInt64]) {
        ...
        if idsToCheck.length == 0 {
            idsToCheck = IntentMarketplaceV0_3.getOpenIntents()
        }
        ...
        for intentID in idsToCheck {
            ...
            if intentRef.status == IntentMarketplaceV0_3.IntentStatus.Active {
                let currentAPY = ScheduledManagerV0_3.fetchCurrentAPY(intentID: intentID)
                let targetAPY  = intentRef.targetAPY
                let threshold  = targetAPY * ScheduledManagerV0_3.rebalanceThreshold
    ...
    access(self) fun fetchCurrentAPY(intentID: UInt64): UFix64 {
        let config = ScheduledManagerV0_3.evmConfig["composerV2"]
        if config == nil {
            return 0.0
        }
    ...
    access(self) fun parseEVMAddress(_ hexAddr: String): EVM.EVMAddress {
        ...
        var i = 0
        while i < 40 {
            let high = ScheduledManagerV0_3.hexCharToUInt8(hex.slice(from: i,     upTo: i + 1))
            let low  = ScheduledManagerV0_3.hexCharToUInt8(hex.slice(from: i + 1, upTo: i + 2))
    ...
    init() {
        ...
        self.AdminStoragePath = /storage/FlowIntentsScheduledAdminV3
