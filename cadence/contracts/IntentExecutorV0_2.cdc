...
access(all) contract IntentExecutorV0_2 {
    ...
    access(all) resource Admin {
        access(all) fun setComposerAddress(addr: String) {
            IntentExecutorV0_2.composerAddress = addr
            // Also update evmConfig
            if let existing = IntentExecutorV0_2.evmConfig["composer"] {
                IntentExecutorV0_2.evmConfig["composer"] = EVMConfig(
                    address: addr,
                    selector: existing.selector
                )
            }
            if let existing = IntentExecutorV0_2.evmConfig["composer_getIntentBalance"] {
                IntentExecutorV0_2.evmConfig["composer_getIntentBalance"] = EVMConfig(
                    address: addr,
                    selector: existing.selector
                )
            }
        ...
        access(all) fun setEVMContract(name: String, config: EVMConfig) {
            IntentExecutorV0_2.evmConfig[name] = config
            // Keep backward-compat var in sync
            if name == "composer" || name == "composer_getIntentBalance" {
                IntentExecutorV0_2.composerAddress = config.address
            }
        ...
        access(all) fun getEVMConfig(name: String): EVMConfig? {
            return IntentExecutorV0_2.evmConfig[name]
        }
    }
    ...
    access(self) fun parseEVMAddress(_ hexAddr: String): EVM.EVMAddress {
        ...
        var i = 0
        while i < 40 {
            let high = IntentExecutorV0_2.hexCharToUInt8(hex.slice(from: i,     upTo: i + 1))
            let low  = IntentExecutorV0_2.hexCharToUInt8(hex.slice(from: i + 1, upTo: i + 2))
    ...
    access(self) fun dryReadPosition(
        ...
        let config = IntentExecutorV0_2.evmConfig["composer_getIntentBalance"]
        ...
        let result = EVM.dryCall(
            from: coaAddress,
            to: IntentExecutorV0_2.parseEVMAddress(config.address),
            data: calldata,
            gasLimit: 50000,
    ...
    access(all) fun executeIntent(
        ...
        pre {
            IntentExecutorV0_2.composerAddress != "0x0000000000000000000000000000000000000000":
                "IntentExecutorV0_2: composerAddress not set — call Admin.setComposerAddress() first"
        }
        ...
        let composerEVMAddress = IntentExecutorV0_2.parseEVMAddress(IntentExecutorV0_2.composerAddress)
        ...
        emit IntentExecuted(
            ...
            composerAddress: IntentExecutorV0_2.composerAddress,
    ...
    access(all) fun completeIntent(
        ...
        pre {
            IntentExecutorV0_2.composerAddress != "0x0000000000000000000000000000000000000000":
                "IntentExecutorV0_2: composerAddress not set"
        }
    ...
    access(all) fun executeIntentV2(
        ...
        assert(
            intent.status == IntentMarketplaceV0_2.IntentStatus.BidSelected,
            message: "Intent must be in BidSelected status for execution"
        )
    ...
    init() {
        ...
        self.AdminStoragePath = /storage/FlowIntentsExecutorAdminV2
