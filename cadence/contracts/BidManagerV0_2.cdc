...
access(all) contract BidManagerV0_2 {
    ...
    access(all) fun submitBid(
        ...
        assert(
            intent.status == IntentMarketplaceV0_2.IntentStatus.Open,
            message: "Intent is not open for bids"
        )

        // Type-specific validation
        let intentType = intent.intentType
        if intentType == IntentMarketplaceV0_2.IntentType.Yield || intentType == IntentMarketplaceV0_2.IntentType.BridgeYield {
            ...
            if intentType == IntentMarketplaceV0_2.IntentType.BridgeYield {
                if let minAPY = intent.minAPY {
                    assert((offeredAPY ?? 0.0) >= minAPY, message: "offeredAPY below intent minAPY")
                }
            }
        } else if intentType == IntentMarketplaceV0_2.IntentType.Swap {
        ...
        assert(
            BidManagerV0_2.solverBidForIntent[dedupeKey] == nil,
        ...
        let bidID = BidManagerV0_2.totalBids
        let bid <- create Bid(
            id: bidID,
            intentID: intentID,
        ...
        BidManagerV0_2.bids[bidID] <-! bid
        BidManagerV0_2.totalBids = BidManagerV0_2.totalBids + 1

        // Index by intent
        if BidManagerV0_2.intentBids[intentID] == nil {
            BidManagerV0_2.intentBids[intentID] = []
        }
        BidManagerV0_2.intentBids[intentID]!.append(bidID)
        BidManagerV0_2.solverBidForIntent[dedupeKey] = bidID
    ...
    access(all) fun selectWinner(intentID: UInt64, callerAddress: Address) {
        ...
        let intent = IntentMarketplaceV0_2.getIntent(id: intentID)
            ?? panic("Intent does not exist")
        assert(intent.status == IntentMarketplaceV0_2.IntentStatus.Open, message: "Intent not open")
        assert(
            intent.intentOwner == callerAddress,
            message: "Only the intent owner can select the winner"
        )

        let bidIDs = BidManagerV0_2.intentBids[intentID] ?? panic("No bids for this intent")
        ...
        for bidID in bidIDs {
            let bid = (&BidManagerV0_2.bids[bidID] as &Bid?)!
        ...
        BidManagerV0_2.winners[intentID] = winningBidID
        ...
        let winningBid = (&BidManagerV0_2.bids[winningBidID] as &Bid?)!
    ...
    access(all) fun getBid(bidID: UInt64): &Bid? {
        return &BidManagerV0_2.bids[bidID] as &Bid?
    }

    access(all) fun getWinningBid(intentID: UInt64): &Bid? {
        if let winnerID = BidManagerV0_2.winners[intentID] {
            return &BidManagerV0_2.bids[winnerID] as &Bid?
        }
        return nil
    }

    access(all) fun getBidsForIntent(intentID: UInt64): [UInt64] {
        return BidManagerV0_2.intentBids[intentID] ?? []
    }

    access(all) fun getWinningBidID(intentID: UInt64): UInt64? {
        return BidManagerV0_2.winners[intentID]
    }

    access(all) fun getEncodedBatch(intentID: UInt64): [UInt8]? {
        if let winnerID = BidManagerV0_2.winners[intentID] {
            if let bid = &BidManagerV0_2.bids[winnerID] as &Bid? {
    ...
    init() {
        ...
        self.BidManagerStoragePath = /storage/FlowIntentsBidManagerV2
        self.BidManagerPublicPath  = /public/FlowIntentsBidManagerV2
    }
}
