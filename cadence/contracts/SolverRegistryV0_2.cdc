...
access(all) contract SolverRegistryV0_2 {
    ...
    // -------------------------------------------------------------------------
    // EVM Selector Registry — configurable EVM addresses and selectors
    // -------------------------------------------------------------------------

    /// EVMConfig holds an EVM contract address and its function selector.
    /// Allows changing EVM contract address OR function selector via admin
    /// transaction without redeploying this Cadence contract.
    ...
    /// Keys:
    ///   "identityRegistry_ownerOf"          -> AgentIdentityRegistry.ownerOf(uint256)
    ///   "reputationRegistry_getMultiplier"   -> AgentReputationRegistry.getMultiplier(uint256)
    ...
    access(all) resource Admin {
        access(all) fun setIdentityRegistry(addr: String) {
            SolverRegistryV0_2.agentIdentityRegistryAddress = addr
            // Also update evmConfig entry
            if let existing = SolverRegistryV0_2.evmConfig["identityRegistry_ownerOf"] {
                SolverRegistryV0_2.evmConfig["identityRegistry_ownerOf"] = EVMConfig(
                    address: addr,
        ...
        access(all) fun setReputationRegistry(addr: String) {
            SolverRegistryV0_2.agentReputationRegistryAddress = addr
            // Also update evmConfig entry
            if let existing = SolverRegistryV0_2.evmConfig["reputationRegistry_getMultiplier"] {
                SolverRegistryV0_2.evmConfig["reputationRegistry_getMultiplier"] = EVMConfig(
                    address: addr,
                    selector: existing.selector
                )
            }
        ...
        /// This allows changing any EVM contract address or function selector
        /// without redeploying this Cadence contract.
        access(all) fun setEVMContract(name: String, config: EVMConfig) {
            SolverRegistryV0_2.evmConfig[name] = config
            // Keep backward-compat vars in sync
            if name == "identityRegistry_ownerOf" {
                SolverRegistryV0_2.agentIdentityRegistryAddress = config.address
            } else if name == "reputationRegistry_getMultiplier" {
                SolverRegistryV0_2.agentReputationRegistryAddress = config.address
            }
        ...
        access(all) fun getEVMConfig(name: String): EVMConfig? {
            return SolverRegistryV0_2.evmConfig[name]
        }
    }
    ...
    /// Encode a staticCall to `ownerOf(uint256 tokenId)` — selector from evmConfig
    access(self) fun encodeOwnerOf(tokenId: UInt256): [UInt8] {
        // Use configurable selector from evmConfig (default: 0x6352211e)
        let config = SolverRegistryV0_2.evmConfig["identityRegistry_ownerOf"]
            ?? panic("EVMConfig not set for identityRegistry_ownerOf")
    ...
    /// Encode a staticCall to `getMultiplier(uint256 tokenId)` — selector from evmConfig
    access(self) fun encodeGetMultiplier(tokenId: UInt256): [UInt8] {
        // Use configurable selector from evmConfig (default: 0xadf8252d)
        let config = SolverRegistryV0_2.evmConfig["reputationRegistry_getMultiplier"]
            ?? panic("EVMConfig not set for reputationRegistry_getMultiplier")
        var calldata: [UInt8] = config.selector
        var tmp = tokenId
        var tokenIdBytes: [UInt8] = []
        var j = 0
    ...
    access(self) fun parseEVMAddress(_ hexAddr: String): EVM.EVMAddress {
        ...
        while i < 40 {
            ...
            let high = SolverRegistryV0_2.hexCharToUInt8(byteStr.slice(from: 0, upTo: 1))
            let low  = SolverRegistryV0_2.hexCharToUInt8(byteStr.slice(from: 1, upTo: 2))
    ...
    access(all) fun registerSolver(
        ...
        // ------------------------------------------------------------------
        let identityAddr = SolverRegistryV0_2.parseEVMAddress(SolverRegistryV0_2.agentIdentityRegistryAddress)
        let ownerOfCalldata = SolverRegistryV0_2.encodeOwnerOf(tokenId: tokenId)

        let identityResult = EVM.dryCall(
            from: SolverRegistryV0_2.parseEVMAddress(evmAddress),
            to: identityAddr,
        ...
        let isNonZero = SolverRegistryV0_2.decodeUInt256(data: ownerData) != 0
        assert(
            isNonZero,
        ...
        // ------------------------------------------------------------------
        let reputationAddr = SolverRegistryV0_2.parseEVMAddress(SolverRegistryV0_2.agentReputationRegistryAddress)
        let multiplierCalldata = SolverRegistryV0_2.encodeGetMultiplier(tokenId: tokenId)
        ...
        if reputationResult.status == EVM.Status.successful && reputationResult.data.length >= 32 {
            let rawMultiplier = SolverRegistryV0_2.decodeUInt256(data: reputationResult.data)
            // rawMultiplier may be WAD-scaled (1e18 = 1.0x) or basis-point-scaled (10000 = 1.0x).
            // WAD values >= 1e15 would overflow UFix64(), so normalize first.
            if rawMultiplier >= 1_000_000_000_000_000 {
                // WAD-scaled: 1e18 → 10000 bp. Divide by 1e14 to get basis points.
                let basisPoints = rawMultiplier / 100_000_000_000_000
                if basisPoints > 0 && basisPoints <= 1_000_000 {
                    reputationMultiplier = UFix64(basisPoints) / 10000.0
                }
            } else if rawMultiplier > 0 && rawMultiplier <= 1_000_000 {
        ...
        assert(
            SolverRegistryV0_2.evmToCadence[evmLower] == nil,
    ...
    access(all) fun registerSolverWithAddress(
        ...
        // ------------------------------------------------------------------
        let evmLower = evmAddress.toLower()
        assert(SolverRegistryV0_2.evmToCadence[evmLower] == nil, message: "EVM address already registered")
        assert(SolverRegistryV0_2.solvers[cadenceAddress] == nil, message: "Cadence address already registered")
        ...
        SolverRegistryV0_2.solvers[cadenceAddress] = info
        SolverRegistryV0_2.evmToCadence[evmLower] = cadenceAddress
    ...
    access(all) fun refreshReputation(
        ...
        assert(SolverRegistryV0_2.solvers[cadenceAddress] != nil, message: "Solver not registered")
        let info = SolverRegistryV0_2.solvers[cadenceAddress]!

        let reputationAddr = SolverRegistryV0_2.parseEVMAddress(SolverRegistryV0_2.agentReputationRegistryAddress)
        let multiplierCalldata = SolverRegistryV0_2.encodeGetMultiplier(tokenId: info.tokenId)

        let result = EVM.dryCall(
            from: SolverRegistryV0_2.parseEVMAddress(info.evmAddress),
            to: reputationAddr,
            data: multiplierCalldata,
            gasLimit: 50000,
            value: EVM.Balance(attoflow: 0)
        ...
        if result.status == EVM.Status.successful && result.data.length >= 32 {
            let rawMultiplier = SolverRegistryV0_2.decodeUInt256(data: result.data)
            var newMultiplier: UFix64 = info.reputationMultiplier
            if rawMultiplier >= 1_000_000_000_000_000 {
                let basisPoints = rawMultiplier / 100_000_000_000_000
                if basisPoints > 0 && basisPoints <= 1_000_000 {
                    newMultiplier = UFix64(basisPoints) / 10000.0
                }
            } else if rawMultiplier > 0 && rawMultiplier <= 1_000_000 {
                newMultiplier = UFix64(rawMultiplier) / 10000.0
            }
            var updated = info
            updated.updateReputation(newMultiplier: newMultiplier)
            SolverRegistryV0_2.solvers[cadenceAddress] = updated
        }
    }
    ...
    access(all) fun getSolver(cadenceAddress: Address): SolverInfo? {
        return SolverRegistryV0_2.solvers[cadenceAddress]
    }

    access(all) fun getSolverByEVM(evmAddress: String): SolverInfo? {
        let evmLower = evmAddress.toLower()
        if let cadenceAddr = SolverRegistryV0_2.evmToCadence[evmLower] {
            return SolverRegistryV0_2.solvers[cadenceAddr]
        }
        return nil
    }

    access(all) fun isRegistered(cadenceAddress: Address): Bool {
        return SolverRegistryV0_2.solvers[cadenceAddress] != nil
    }

    access(all) fun getReputationMultiplier(cadenceAddress: Address): UFix64 {
        if let info = SolverRegistryV0_2.solvers[cadenceAddress] {
    ...
    access(all) fun getAllSolverAddresses(): [Address] {
        return SolverRegistryV0_2.solvers.keys
    }
    ...
    init() {
        ...
        self.evmConfig = {
            "identityRegistry_ownerOf": EVMConfig(
                address: "0x0000000000000000000000000000000000000000",
                selector: [0x63, 0x52, 0x21, 0x1e]  // ownerOf(uint256)
            ),
            "reputationRegistry_getMultiplier": EVMConfig(
                address: "0x0000000000000000000000000000000000000000",
                selector: [0xad, 0xf8, 0x25, 0x2d]  // getMultiplier(uint256)
            )
        }
        ...
        self.RegistryStoragePath = /storage/FlowIntentsSolverRegistryV2
        self.RegistryPublicPath  = /public/FlowIntentsSolverRegistryV2
        self.AdminStoragePath    = /storage/FlowIntentsSolverRegistryAdminV2
