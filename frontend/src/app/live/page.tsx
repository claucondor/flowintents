...
> = {
  ...
  BidSubmitted:    { label: "BID",       color: "var(--text-primary)", dot: "var(--text-muted)", bg: "rgba(255,255,255,0.03)" },
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <button
            onClick={() => load(true)}
            className={cn(
              "p-2 border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-[#0047FF]/40 transition-all",
        ...
        <motion.div
          ...
          className="grid grid-cols-3 sm:grid-cols-6 gap-0 border border-[var(--border)] mb-6"
        >
          {[
            { label: "Events",   value: loading ? "—" : stats.total,              color: "var(--text-primary)" },
            { label: "Intents",  value: loading ? "—" : stats.intents,            color: "#0047FF" },
            { label: "Bids",     value: loading ? "—" : stats.bids,               color: "var(--text-primary)" },
            { label: "Winners",  value: loading ? "—" : stats.winners,            color: "#F5C542" },
            { label: "Executed", value: loading ? "—" : stats.executed,           color: "#00C566" },
            { label: "Volume",   value: loading ? "—" : `${fmtFlow(stats.volume)}`, color: "var(--text-primary)" },
          ].map((s, i) => (
            <div
              key={s.label}
              className={`p-4 ${i < 5 ? "border-r border-[var(--border)]" : ""}`}
              style={{ background: "var(--bg-card)" }}
            >
              <div
        ...
        <motion.div
          ...
          className="flex items-center gap-0 border-b border-[var(--border)] mb-6"
        >
          ...
          <span
            className="ml-auto pr-4 text-[10px] text-[var(--text-muted)]"
            style={{ fontFamily: "'Space Mono', monospace" }}
          >
        ...
        >
          {loading ? (
            <div className="border border-[var(--border)]" style={{ background: "var(--bg-card)" }}>
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="px-6 py-4 border-b border-[var(--border)] last:border-0">
                  <div className="flex items-start gap-4">
                    <div className="w-16 h-4 bg-[var(--border)] rounded animate-pulse shrink-0 mt-0.5" />
                    ...
                    <div className="w-20 h-3 bg-[var(--border)] rounded animate-pulse shrink-0" />
                  </div>
                </div>
              ))}
            </div>
                ...
                {filtered.map((evt) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="px-6 py-4 border-b border-[var(--border)] last:border-0 group transition-colors"
                      style={{
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          {evt.transactionId ? (
                            <a
                              ...
                              className="inline-flex items-center gap-1 text-[10px] text-[var(--text-muted)] hover:text-[#0047FF] transition-colors mt-0.5"
                              style={{ fontFamily: "'Space Mono', monospace" }}
                            >
        ...
        {!loading && events.length > 0 && (
          <div
            className="text-center mt-6 text-[10px] text-[var(--text-muted)]"
            style={{ fontFamily: "'Space Mono', monospace" }}
          >
